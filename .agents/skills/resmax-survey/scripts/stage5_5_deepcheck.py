...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--enable-sci-hub", action="store_true",
                   help="Enable Sci-Hub gray fallback. Default is off; use only with explicit user approval.")
    ...
    for cand in selected:
        ...
        if not has_readable:
            missing_source.append({
                ...
                "source_text_status": cand.source_text_status,
                "source_text_url": cand.source_text_url,
                "source_text_source": cand.source_text_source,
        ...
        if "pdf" not in desc["sources_present"]:
            ...
            #   no_oa_copy_found : OA APIs unanimously is_oa=False. If the
            #                      explicit Sci-Hub fallback was enabled, it
            #                      also reported no entry. This is a documented
            #                      dead-end, not a retriable error.
            ...
            if oa_ran and (oa_info.get("is_oa_any") is False) and ((not args.enable_sci_hub) or (sh_ran and not sh_info.get("ok"))):
                category = "no_oa_copy_found"
                hint = (
                    "All legal OA aggregators agree no author self-archived copy "
                    "exists. Sci-Hub was not used unless explicitly enabled. This paper may have "
            ...
            missing_pdf.append({
                ...
                "source_text_url": cand.source_text_url,
                "source_text_source": cand.source_text_source,
                "source_text_search_query": cand.source_text_search_query,
