...
from data_contracts import normalize_repo_url, repo_cache_key  # noqa: E402
...
def _parse_owner_repo(code_url: str) -> Optional[tuple[str, str]]:
    code_url = normalize_repo_url(code_url)
    m = re.match(
...
def main():
    ...
    for i, row in enumerate(rows):
        if conf_filter and conf_filter not in row.get("conf_year", ""):
            continue
        code_url = normalize_repo_url(row.get("code_url", ""))
        if not code_url:
            continue
    ...
    for row_idx, owner, repo in targets:
        cache_key = repo_cache_key(f"https://github.com/{owner}/{repo}")
