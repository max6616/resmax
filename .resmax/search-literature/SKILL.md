...
- `paper_database/accepted_index.csv` 已存在（由 build-literature-base 产出）
- `paper_database/embedding_cache/qwen3_8b.npz` 已存在（由 build-literature-base 产出）
...
```bash
python3 .resmax/search-literature/scripts/search_literature.py \
...
在 `accepted_index.csv` 基础字段（见 [build-literature-base/SKILL.md](../build-literature-base/SKILL.md)）之上，增加以下方向级检索与评分字段：
