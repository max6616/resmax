...
```bash
python3 .resmax/build-literature-base/scripts/survey_sources.py \
  --venues ICLR,CVPR \
  --years 2025,2026 \
...
```bash
python3 .resmax/build-literature-base/scripts/build_accepted_index.py \
  --registry .resmax/build-literature-base/config/source_registry.json \
...
| 参数 | 必填 | 说明 | 示例 |
|------|------|------|------|
| `--registry` | 是 | 数据源注册表 JSON 路径 | `.resmax/build-literature-base/config/source_registry.json` |
...
```bash
python3 .resmax/build-literature-base/scripts/enrich_abstracts.py \
...
```bash
python3 .resmax/build-literature-base/scripts/enrich_abstracts_fallback.py \
  --csv paper_database/accepted_index.csv \
  --filter <CONF_YEAR>
```
...
# 构建缓存（指定空闲 GPU）
python3 .resmax/build-literature-base/scripts/build_cache_multigpu.py \
