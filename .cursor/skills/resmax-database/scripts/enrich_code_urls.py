...
def _normalize_repo_url(url: str) -> str:
    """Normalize a GitHub/GitLab URL to canonical form."""
    url = url.strip().rstrip("/")
    for suffix in (".git", "/tree/master", "/tree/main", "/blob/master", "/blob/main"):
    ...
    if "github.com" in url or "gitlab.com" in url:
        host_idx = next(
            (i for i, p in enumerate(parts) if p in ("github.com", "gitlab.com")),
            None,
        )
        if host_idx is not None and len(parts) > host_idx + 2:
            parts = parts[: host_idx + 3]
    return "/".join(parts)
        ...
        if paper_url and paper_url in pwc_links:
            ...
            for r in repos:
                if "github.com" in r:
                    best = r
                    break
            row["code_url"] = _normalize_repo_url(best)
            enriched += 1
...
def _extract_github_from_s2(paper: dict) -> Optional[str]:
    ...
    if github_url:
        ...
        return _normalize_repo_url(github_url)
    return None
...
def _match_regex(rows: list[dict]) -> int:
    ...
    for row in rows:
        ...
        for pattern in (GITHUB_URL_RE, GITLAB_URL_RE):
            ...
            if m:
                full_url = m.group(0)
                normalized = _normalize_repo_url(full_url)
